...
import { ChevronRight, Home, FileText, Pencil } from "lucide-react";
...
export default async function EditarCotizacionPage({
  ...
  if (!editDocument || editDocument.tipo !== "COTIZACION") notFound();
  ...
  return (
    <div className="fade-up space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        <div>
          ...
          <p className="mt-1.5 text-[var(--text-2)] text-sm">
            Modifica los datos de la cotización. Los cambios reemplazarán los ítems y totales actuales.
          </p>
        </div>
        <Link
          href={`/documentos/${id}`}
      ...
      <CotizacionForm tipoDocumento="COTIZACION" seller={seller} editDocument={editDocument} />
    </div>
  );
}
